    ...
    <div style="min-height: 600px">
      ...
      <div v-if="$page.allGist.edges.length > 0">
        <el-card shadow="hover" v-for="item in $page.allGist.edges" :key="item.node.id" style="margin-bottom: 20px">
          ...
          <div style="font-size: 1.1rem;line-height: 1.5;color: #303133;padding: 10px 0px 0px 0px" v-html="mdToHtml(item.node.content)"></div>
        </el-card>
...
<page-query>
query($page: Int){
  allGist:allStrapiGist(perPage: 1, page: $page) @paginate{
    pageInfo {
      totalPages
      currentPage
      totalItems
