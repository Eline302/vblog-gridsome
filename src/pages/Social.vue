        ...
        <el-tabs v-model="activeTab" type="card" @tab-click="handleTabChange">
            <el-tab-pane :label="`粉丝${$page.allFollowers.pageInfo.totalItems}`" name="followers" style="padding: 5px">
              <div v-if="$page.allFollowers.edges.length">
                <el-row style="min-height: 200px; ">
                  <el-col :span="8" v-for="item in $page.allFollowers.edges" :key="item.node.id" style="padding: 10px">
                    <el-card shadow="hover" style="font-size: 13px;color: #606266;line-height: 20px">
                      ...
                      <a @click="$router.push(`/user/${item.node.name}`)" style="text-decoration:none;cursor:pointer;color: #1e6bb8;">{{item.node.name}}</a>
                      <br />
                ...
                <div style="text-align: center;margin-top: 10px" v-if="$page.allFollowing.pageInfo.totalItems>10">
                  <el-pagination
...
<page-query>
query ($page: Int) {
  allFollowers:allStrapiFollower (perPage: 10, page: $page) @paginate {
    pageInfo {
      currentPage
      perPage
      totalItems
  ...
  allFollowing:allStrapiFollowing (perPage: 10, page: $page) @paginate {
    pageInfo {
      currentPage
      perPage
      totalItems
